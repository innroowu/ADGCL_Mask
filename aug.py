...
def neighbor_pruning(graph, node_dist, sim, x, degree, feat_drop_rate_1, feat_drop_rate_2, threshold):
    ...
    # 修復CUDA張量轉換問題
    src_nodes_cpu = src_nodes.cpu().numpy()
    dst_nodes_cpu = dst_nodes.cpu().numpy()
    src_idx_cpu = src_idx.cpu().numpy()
    
    src_edge_indices = np.where(np.isin(src_nodes_cpu, src_idx_cpu))[0]
    dst_edge_indices = np.where(np.isin(dst_nodes_cpu, src_idx_cpu))[0]

    combined_indices = np.unique(np.concatenate((src_edge_indices, dst_edge_indices)))
    new_row_mix, new_col_mix = src_nodes_cpu[combined_indices], dst_nodes_cpu[combined_indices]

    new_row_rest, new_col_rest = degree_mask_edge_threshold(node_dist, rest_idx, sim, max_degree, rest_node_degree, threshold)

    nsrc = np.concatenate((new_row_mix, new_row_rest.cpu().numpy()))
    ndst = np.concatenate((new_col_mix, new_col_rest.cpu().numpy()))
    
    ng = dgl.graph((nsrc, ndst), num_nodes=graph.number_of_nodes())
    ng = dgl.remove_self_loop(ng)
    ng = dgl.add_self_loop(ng)

    # 修復鄰接矩陣取得問題
    adj = get_adjacency_matrix_for_aug(ng)
    adj = sp.csr_matrix(adj.cpu().numpy())
    adj = normalize_adj(adj)
    adj = (adj + sp.eye(adj.shape[0])).todense()
    adj = th.FloatTensor(adj[np.newaxis])
...
def neighbor_completion(graph, node_dist, sim, ano_sim, x, degree, 
    ...
    ng1 = dgl.remove_self_loop(ng1)
    ng1 = dgl.add_self_loop(ng1)
    ...
    ng2 = dgl.remove_self_loop(ng2)
    ng2 = dgl.add_self_loop(ng2)

    # 修復鄰接矩陣取得問題
    adj1 = get_adjacency_matrix_for_aug(ng1)
    adj1 = sp.csr_matrix(adj1.cpu().numpy())
    adj1 = normalize_adj(adj1)
    adj1 = (adj1 + sp.eye(adj1.shape[0])).todense()
    adj1 = th.FloatTensor(adj1[np.newaxis])

    adj2 = get_adjacency_matrix_for_aug(ng2)
    adj2 = sp.csr_matrix(adj2.cpu().numpy())
    adj2 = normalize_adj(adj2)
    adj2 = (adj2 + sp.eye(adj2.shape[0])).todense()
    adj2 = th.FloatTensor(adj2[np.newaxis])

    return ng1, ng2, feat1.unsqueeze(0), feat2.unsqueeze(0), adj1, adj2


def get_adjacency_matrix_for_aug(graph):
    """為aug.py專用的鄰接矩陣取得函數"""
    try:
        num_nodes = graph.number_of_nodes()
        adj_tensor = th.zeros((num_nodes, num_nodes))
        src, dst = graph.edges()
        adj_tensor[src, dst] = 1.0
        return adj_tensor
    except Exception as e:
        print(f"Error creating adjacency matrix in aug.py: {e}")
        return th.zeros((graph.number_of_nodes(), graph.number_of_nodes()))
